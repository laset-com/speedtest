...
TEMP_DIR=$(mktemp -d -t speedtest-XXXXXX) || error_exit "Failed to create general temporary directory."
GEEKBENCH_BASE_DIR=""
RAMDISK_BASE_DIR=""
...
benchinit() {
    # Create unique temporary directories (TEMP_DIR and log are already set globally)
    GEEKBENCH_BASE_DIR=$(mktemp -d -t geekbench-XXXXXX) || error_exit "Failed to create Geekbench temporary directory."
    RAMDISK_BASE_DIR=$(mktemp -d -t ramdisk-XXXXXX) || error_exit "Failed to create RAM disk temporary directory."
    ...
    install_package() {
        ...
        if [ ! -e "$package_path" ]; then
            ...
            if [[ "${release}" == "centos" || "${release}" == "almalinux" || "${release}" == "rocky" ]]; then # Added rocky
                dnf -y install "$package_name" > "$NULL" 2>&1 || yum -y install "$package_name" > "$NULL" 2>&1
            elif [[ "${release}" == "debian" || "${release}" == "ubuntu" ]]; then
                apt-get update -y > "$NULL" 2>&1
                apt-get -y install "$package_name" > "$NULL" 2>&1
            else
                echo " Unknown distribution, trying apt-get and yum..."
                apt-get -y install "$package_name" > "$NULL" 2>&1 || yum -y install "$package_name" > "$NULL" 2>&1
            fi
            echo -ne "\e[1A"; echo -ne "\e[0K\r"
        fi
    }
    ...
    if [[ "${release}" == "centos" || "${release}" == "almalinux" || "${release}" == "rocky" ]] && [ -e '/usr/bin/python3' ]; then # Added rocky
        alternatives --set python3 /usr/bin/python3 > "$NULL" 2>&1 || true
    fi
    ...
    if  [ ! -e "$TEMP_DIR/speedtest.py" ]; then
        echo " Installing Speedtest-cli ..."
        wget --no-check-certificate https://raw.githubusercontent.com/laset-com/speedtest-cli/master/speedtest.py -O "$TEMP_DIR/speedtest.py" > "$NULL" 2>&1
        echo -ne "\e[1A"; echo -ne "\e[0K\r"
    fi
    chmod a+rx "$TEMP_DIR/speedtest.py"


    # install tools.py
    if  [ ! -e "$TEMP_DIR/tools.py" ]; then
        echo " Installing tools.py ..."
        wget --no-check-certificate https://raw.githubusercontent.com/laset-com/speedtest/master/tools.py -O "$TEMP_DIR/tools.py" > "$NULL" 2>&1
        echo -ne "\e[1A"; echo -ne "\e[0K\r"
    fi
    chmod a+rx "$TEMP_DIR/tools.py"
...
geekbench4() {
    ...
    GEEKBENCH_PATH="$GEEKBENCH_BASE_DIR/geekbench"
    mkdir -p "$GEEKBENCH_PATH"
    curl -s https://cdn.geekbench.com/Geekbench-4.4.4-Linux.tar.gz  | tar xz --strip-components=1 -C "$GEEKBENCH_PATH" &>"$NULL"
    GEEKBENCH_TEST=$("$GEEKBENCH_PATH/geekbench4" 2>"$NULL" | grep "https://browser")
    ...
    [ ! -z "$GEEKBENCH_URL_CLAIM" ] && echo -e "$GEEKBENCH_URL_CLAIM" >> "$TEMP_DIR/geekbench_claim.url" 2>"$NULL"
    echo "" | tee -a "$log"
    echo -e " Cooling down..."
    sleep 9
    echo -ne "\e[1A"; echo -ne "\033[0K\r"
...
geekbench5() {
    ...
    if [[ $(uname -m) == "aarch64" || $(uname -m) == "arm64" ]]; then
        curl -s https://cdn.geekbench.com/Geekbench-5.5.1-LinuxARMPreview.tar.gz | tar xz --strip-components=1 -C "$GEEKBENCH_PATH" &>"$NULL"
    elif [[ $(uname -m) == "riscv64" ]]; then
        curl -s https://cdn.geekbench.com/Geekbench-5.5.1-LinuxRISCVPreview.tar.gz | tar xz --strip-components=1 -C "$GEEKBENCH_PATH" &>"$NULL"
    else
        curl -s https://cdn.geekbench.com/Geekbench-5.5.1-Linux.tar.gz | tar xz --strip-components=1 -C "$GEEKBENCH_PATH" &>"$NULL"
    fi
    GEEKBENCH_TEST=$("$GEEKBENCH_PATH/geekbench5" 2>"$NULL" | grep "https://browser")
    GEEKBENCH_URL=$(echo -e "$GEEKBENCH_TEST" | head -1)
    GEEKBENCH_URL_CLAIM=$(echo "$GEEKBENCH_URL" | awk '{ print $2 }')
    GEEKBENCH_URL=$(echo "$GEEKBENCH_URL" | awk '{ print $1 }')
...
geekbench6() {
    ...
    mkdir -p "$GEEKBENCH_PATH"
    if [[ $(uname -m) == "aarch64" || $(uname -m) == "arm64" ]]; then
        curl -s https://cdn.geekbench.com/Geekbench-6.5.0-LinuxARMPreview.tar.gz | tar xz --strip-components=1 -C "$GEEKBENCH_PATH" &>"$NULL"
    elif [[ $(uname -m) == "riscv64" ]]; then
        curl -s https://cdn.geekbench.com/Geekbench-6.5.0-LinuxRISCVPreview.tar.gz | tar xz --strip-components=1 -C "$GEEKBENCH_PATH" &>"$NULL"
    else
        curl -s https://cdn.geekbench.com/Geekbench-6.5.0-Linux.tar.gz | tar xz --strip-components=1 -C "$GEEKBENCH_PATH" &>"$NULL"
    fi
    GEEKBENCH_TEST=$("$GEEKBENCH_PATH/geekbench6" 2>"$NULL" | grep "https://browser")
    GEEKBENCH_URL=$(echo -e "$GEEKBENCH_TEST" | head -1)
    GEEKBENCH_URL_CLAIM=$(echo "$GEEKBENCH_URL" | awk '{ print $2 }')
    GEEKBENCH_URL=$(echo "$GEEKBENCH_URL" | awk '{ print $1 }')
    ...
    echo -e "   Multi Core : $GEEKBENCH_SCORES_MULTI" | tee -a "$log"
    echo -e "    CPU Steal : ${STEAL_PERCENT}%" | tee -a "$log"
    [ ! -z "$GEEKBENCH_URL_CLAIM" ] && echo -e "$TEMP_DIR/geekbench_claim.url" 2>"$NULL"
    echo "" | tee -a "$log"
    echo -e " Cooling down..."
    sleep 9
    echo -ne "\e[1A"; echo -ne "\033[0K\r"
...
ip_info4(){
    isp=$(python3 "$TEMP_DIR/tools.py" geoip isp)
    as_tmp=$(python3 "$TEMP_DIR/tools.py" geoip as)
    asn=$(echo "$as_tmp" | awk -F ' ' '{print $1}')
    org=$(python3 "$TEMP_DIR/tools.py" geoip org)
    country=$(python3 "$TEMP_DIR/tools.py" geoip country)
    city=$(python3 "$TEMP_DIR/tools.py" geoip city)
    #countryCode=$(python3 "$TEMP_DIR/tools.py" geoip countryCode)
    region=$(python3 "$TEMP_DIR/tools.py" geoip regionName)
