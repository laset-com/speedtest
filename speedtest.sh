...
cleanup() {
	...
	rm -f speedtest.py tools.py 2>"$NULL"
	rm -f ip_json.json 2>"$NULL"
	rm -f geekbench_claim.url 2>"$NULL"
	rm -f test_file_* 2>"$NULL"
	
	# Remove temporary directories created by mktemp
	if [[ -n "$GEEKBENCH_TEMP_DIR" && -d "$GEEKBENCH_TEMP_DIR" ]]; then
		rm -rf "$GEEKBENCH_TEMP_DIR" 2>"$NULL"
	fi
	if [[ -n "$RAMDISK_TEMP_DIR" && -d "$RAMDISK_TEMP_DIR" ]]; then
		umount "$RAMDISK_TEMP_DIR" 2>"$NULL" # Attempt to unmount before removing
		rm -rf "$RAMDISK_TEMP_DIR" 2>"$NULL"
	fi
	
	# Remove the old fixed 'geekbench' directory if it exists (for backward compatibility)
	rm -rf geekbench 2>"$NULL"
}
...
cpubench() {
	if hash "$1" 2>"$NULL"; then
		...
		local io=$( ( dd if=/dev/zero bs=512K count="$2" | "$1" ) 2>&1 | grep 'copied' | awk -F, '{io=$NF} END {print io}' )
...
averageio() {
	local ioraw1=$( echo "$1" | awk 'NR==1 {print $1}' )
		[ "$(echo "$1" | awk 'NR==1 {print $2}')" == "GB/s" ] && ioraw1=$( awk 'BEGIN{printf "%.1f", '$ioraw1' * 1024}' )
	local ioraw2=$( echo "$2" | awk 'NR==1 {print $1}' )
		[ "$(echo "$2" | awk 'NR==1 {print $2}')" == "GB/s" ] && ioraw2=$( awk 'BEGIN{printf "%.1f", '$ioraw2' * 1024}' )
	local ioraw3=$( echo "$3" | awk 'NR==1 {print $1}' )
		[ "$(echo "$3" | awk 'NR==1 {print $2}')" == "GB/s" ] && ioraw3=$( awk 'BEGIN{printf "%.1f", '$ioraw3' * 1024}' )
...
freedisk() {
	...
	local freespace=$( df -m . | awk 'NR==2 {print $4}' )
	if [[ "$freespace" == "" ]]; then
...
power_time() {
	local result=$(smartctl -a "$(result=$(cat /proc/mounts) && echo "$(echo "$result" | awk '/data=ordered/{print $1}')" | awk '{print $1}')" 2>&1) && local power_time=$(echo "$result" | awk '/Power_On/{print $10}') && echo "$power_time"
}
...
speed_test(){
	if [[ "$1" == '' ]]; then
		local temp=$(python3 speedtest.py --secure --share 2>&1)
		local is_down=$(echo "$temp" | grep 'Download')
		local result_speed=$(echo "$temp" | awk -F ' ' '/results/{print $3}')
		if [[ ${is_down} ]]; then
	        local REDownload=$(echo "$temp" | awk -F ':' '/Download/{print $2}')
	        local reupload=$(echo "$temp" | awk -F ':' '/Upload/{print $2}')
	        local relatency=$(echo "$temp" | awk -F ':' '/Hosted/{print $2}')

	        local temp_latency=$(echo "$relativity" | awk -F '.' '{print $1}')
        	if [[ "${temp_latency}" -gt 50 ]]; then
            	relatency="*"${relativity}
        	fi
	        local nodeName=$2

	        local temp_download=$(echo "${REDownload}" | awk -F ' ' '{print $1}')
	        if [[ $(awk -v num1="${temp_download}" -v num2=0 'BEGIN{print(num1>num2)?"1":"0"}') -eq 1 ]]; then
		...
		local temp=$(python3 speedtest.py --secure --server "$1" --share 2>&1)
		local is_down=$(echo "$temp" | grep 'Download') 
		if [[ ${is_down} ]]; then
	        local REDownload=$(echo "$temp" | awk -F ':' '/Download/{print $2}')
	        local reupload=$(echo "$temp" | awk -F ':' '/Upload/{print $2}')
	        ...
	        #local temp_latency=$(echo "$relatity" | awk -F '.' '{print $1}')
        	#if [[ "${temp_latency}" -gt 1000 ]]; then
            	#relatency=" - "
        	#fi
	        local nodeName=$2
...
ip_info(){
	...
	local city=$(curl -s https://ipapi.co/city/)
	local asn=$(curl -s https://ipapi.co/asn/)
	local org=$(curl -s https://ipapi.co/org/)
	local countryCode=$(curl -s https://ipapi.co/country/)
	local region=$(curl -s https://ipapi.co/region/)
...
ip_info4(){
	local isp=$(python3 tools.py geoip isp)
	local as_tmp=$(python3 tools.py geoip as)
	local asn=$(echo "$as_tmp" | awk -F ' ' '{print $1}')
	local org=$(python3 tools.py geoip org)
	local country=$(python3 tools.py geoip country)
	local city=$(python3 tools.py geoip city)
	#countryCode=$(python3 tools.py geoip countryCode)
	local region=$(python3 tools.py geoip regionName)
...
geekbench4() {
	...
	echo -e "\nGeekbench 4 cannot run on 32-bit architectures. Skipping the test" | tee -a "$log"
	elif [[ "$ARCH" = *aarch64* || "$ARCH" = *arm64* ]]; then # ARM64
	echo -e "\nGeekbench 4 is not compatible with ARM64 architectures. Skipping the test" | tee -a "$log"
	else
	echo "" | tee -a "$log"
	...
	curl -s https://cdn.geekbench.com/Geekbench-4.4.4-Linux.tar.gz  | tar xz --strip-components=1 -C "$GEEKBENCH_TEMP_DIR" &>"$NULL"
	local GEEKBENCH_TEST=$("$GEEKBENCH_TEMP_DIR"/geekbench4 2>"$NULL" | grep "https://browser")
	GEEKBENCH_URL=$(echo -e "$GEEKBENCH_TEST" | head -1)
	local GEEKBENCH_URL_CLAIM=$(echo "$GEEKBENCH_URL" | awk '{ print $2 }')
	GEEKBENCH_URL=$(echo "$GEEKBENCH_URL" | awk '{ print $1 }')
	sleep 20
	local GEEKBENCH_SCORES=$(curl -s "$GEEKBENCH_URL" | grep "span class='score'")
	local GEEKBENCH_SCORES_SINGLE=$(echo "$GEEKBENCH_SCORES" | awk -v FS="(>|<)" '{ print $3 }')
	local GEEKBENCH_SCORES_MULTI=$(echo "$GEEKBENCH_SCORES" | awk -v FS="(>|<)" '{ print $7 }')
	...
	[ ! -z "$GEEKBENCH_URL_CLAIM" ] && echo -e "$GEEKBENCH_URL_CLAIM" >> geekbench_claim.url 2> "$NULL"
	echo "" | tee -a "$log"
	echo -e " Cooling down..." | tee -a "$log"
	sleep 9
	echo -ne "\e[1A"; echo -ne "\033[0K\r"
	echo -e " Ready to continue..." | tee -a "$log"
	sleep 3
	echo -ne "\e[1A"; echo -ne "\033[0K\r"
	fi
}
...
geekbench5() {
	if [[ "$ARCH" = *x86* ]]; then # 32-bit
	echo -e "\nGeekbench 5 cannot run on 32-bit architectures. Skipping the test" | tee -a "$log"
	else
	echo "" | tee -a "$log"
	...
	GEEKBENCH_TEMP_DIR=$(mktemp -d -t geekbench.XXXXXX) # Use global variable and specific prefix
	if [[ "$(uname -m)" == "aarch64" || "$(uname -m)" == "arm64" ]]; then
		curl -s https://cdn.geekbench.com/Geekbench-5.5.1-LinuxARMPreview.tar.gz | tar xz --strip-components=1 -C "$GEEKBENCH_TEMP_DIR" &>"$NULL"
	elif [[ "$(uname -m)" == "riscv64" ]]; then
		curl -s https://cdn.geekbench.com/Geekbench-5.5.1-LinuxRISCVPreview.tar.gz | tar xz --strip-components=1 -C "$GEEKBENCH_TEMP_DIR" &>"$NULL"
	else
		curl -s https://cdn.geekbench.com/Geekbench-5.5.1-Linux.tar.gz | tar xz --strip-components=1 -C "$GEEKBENCH_TEMP_DIR" &>"$NULL"
	fi
	local GEEKBENCH_TEST=$("$GEEKBENCH_TEMP_DIR"/geekbench5 2>"$NULL" | grep "https://browser")
	GEEKBENCH_URL=$(echo -e "$GEEKBENCH_TEST" | head -1)
	local GEEKBENCH_URL_CLAIM=$(echo "$GEEKBENCH_URL" | awk '{ print $2 }')
	GEEKBENCH_URL=$(echo "$GEEKBENCH_URL" | awk '{ print $1 }')
	sleep 20
	local GEEKBENCH_SCORES=$(curl -s "$GEEKBENCH_URL" | grep "div class='score'")
	local GEEKBENCH_SCORES_SINGLE=$(echo "$GEEKBENCH_SCORES" | awk -v FS="(>|<)" '{ print $3 }')
	local GEEKBENCH_SCORES_MULTI=$(echo "$GEEKBENCH_SCORES" | awk -v FS="(<|>)" '{ print $7 }')
...
geekbench6() {
	if [[ "$ARCH" = *x86* ]]; then # 32-bit
	echo -e "\nGeekbench 6 cannot run on 32-bit architectures. Skipping the test" | tee -a "$log"
	else
	echo "" | tee -a "$log"
	...
	GEEKBENCH_TEMP_DIR=$(mktemp -d -t geekbench.XXXXXX) # Use global variable and specific prefix
	if [[ "$(uname -m)" == "aarch64" || "$(uname -m)" == "arm64" ]]; then
		curl -s https://cdn.geekbench.com/Geekbench-6.5.0-LinuxARMPreview.tar.gz | tar xz --strip-components=1 -C "$GEEKBENCH_TEMP_DIR" &>"$NULL"
	elif [[ "$(uname -m)" == "riscv64" ]]; then
		curl -s https://cdn.geekbench.com/Geekbench-6.5.0-LinuxRISCVPreview.tar.gz | tar xz --strip-components=1 -C "$GEEKBENCH_TEMP_DIR" &>"$NULL"
	else
		curl -s https://cdn.geekbench.com/Geekbench-6.5.0-Linux.tar.gz | tar xz --strip-components=1 -C "$GEEKBENCH_TEMP_DIR" &>"$NULL"
	fi
	local GEEKBENCH_TEST=$("$GEEKBENCH_TEMP_DIR"/geekbench6 2>"$NULL" | grep "https://browser")
	GEEKBENCH_URL=$(echo -e "$GEEKBENCH_TEST" | head -1)
	local GEEKBENCH_URL_CLAIM=$(echo "$GEEKBENCH_URL" | awk '{ print $2 }')
	GEEKBENCH_URL=$(echo "$GEEKBENCH_URL" | awk '{ print $1 }')
	sleep 15
...
geekbench() {
	local totalram="$( free -m | grep Mem | awk 'NR=1 {print $2}' )"
...
iotest() {
	...
	local tram_mb="$( free -m | grep Mem | awk 'NR=1 {print $2}' )"
	local sbram=""
	local sbcount=""
	...
	echostyle "RAM Speed:"
	local iow1=$( ( dd if=/dev/zero of="$RAMDISK_TEMP_DIR"/zero bs=512K count="$sbcount" ) 2>&1 | awk -F, '{io=$NF} END { print io}' )
	local ior1=$( ( dd if="$RAMDISK_TEMP_DIR"/zero of="$NULL" bs=512K count="$sbcount"; rm -f test ) 2>&1 | awk -F, '{io=$NF} END { print io}' )
	local iow2=$( ( dd if=/dev/zero of="$RAMDISK_TEMP_DIR"/zero bs=512K count="$sbcount" ) 2>&1 | awk -F, '{io=$NF} END { print io}' )
	local ior2=$( ( dd if="$RAMDISK_TEMP_DIR"/zero of="$NULL" bs=512K count="$sbcount"; rm -f test ) 2>&1 | awk -F, '{io=$NF} END { print io}' )
	local iow3=$( ( dd if=/dev/zero of="$RAMDISK_TEMP_DIR"/zero bs=512K count="$sbcount" ) 2>&1 | awk -F, '{io=$NF} END { print io}' )
	local ior3=$( ( dd if="$RAMDISK_TEMP_DIR"/zero of="$NULL" bs=512K count="$sbcount"; rm -f test ) 2>&1 | awk -F, '{io=$NF} END { print io}' )
...
write_io() {
	...
	if [[ "$writemb" != "1" ]]; then
		...
		local io1=$( write_test "$writemb" )
		echo -e "$io1" | tee -a "$log"
		echo -n "   2nd run    : " | tee -a "$log"
		local io2=$( write_test "$writemb" )
		echo -e "$io2" | tee -a "$log"
		echo -n "   3rd run    : " | tee -a "$log"
		local io3=$( write_test "$writemb" )
		echo -e "$io3" | tee -a "$log"
		local ioraw1=$( echo "$io1" | awk 'NR==1 {print $1}' )
		[ "`echo "$io1" | awk 'NR==1 {print $2}'`" == "GB/s" ] && ioraw1=$( awk 'BEGIN{printf "%.1f", '$ioraw1' * 1024}' )
		local ioraw2=$( echo "$io2" | awk 'NR==1 {print $1}' )
		[ "`echo "$io2" | awk 'NR==1 {print $2}'`" == "GB/s" ] && ioraw2=$( awk 'BEGIN{printf "%.1f", '$ioraw2' * 1024}' )
		local ioraw3=$( echo "$io3" | awk 'NR==1 {print $1}' )
		[ "`echo "$io3" | awk 'NR==1 {print $2}'`" == "GB/s" ] && ioraw3=$( awk 'BEGIN{printf "%.1f", '$ioraw3' * 1024}' )
		local ioall=$( awk "BEGIN{print $ioraw1 + $ioraw2 + $ioraw3}" )
		local ioavg=$( awk "BEGIN{printf \"%.1f\", $ioall / 3}" )
		echo -e "   -----------------------" | tee -a "$log"
		echo -e "   Average    : $ioavg MB/s" | tee -a "$log"
	else
		echo -e " Not enough space!" | tee -a "$log"
	fi
}
...
log_preupload() {
	local log_up="$HOME/speedtest_upload.log"
	true > "$log_up"
	$(cat speedtest.log 2>&1 | sed -r "s/\x1B\[([0-9]{1,2}(;[0-9]{1,2})?)?[m|K]//g" > "$log_up")
}

sharetest() {
	echo " Share results:" | tee -a "$log"
	echo " - $result_speed" | tee -a "$log"
	log_preupload
	# Removed commented out share services as they are not implemented
	# case $1 in
	# 'ubuntu')
...
get_ip_whois_org_name(){
	#ip=$(curl -s ip.sb)
	local result=$(curl -s https://rest.db.ripe.net/search.json?query-string=$(curl -s ip.sb))
	#org_name=$(echo "$result" | jq '.objects.object.[1].attributes.attribute.[1].value' | sed 's/\"//g')
	local org_name=$(echo "$result" | jq '.objects.object[1].attributes.attribute[1]' | sed 's/\"//g')
    echo "$org_name";
}

# --- Print Speedtest Functions ---
...
# --- Main Benchmark Execution Functions ---
...
# --- Global Variables and Traps (defined after functions for proper scope) ---

NULL="/dev/null" # Define NULL early
log="$HOME/speedtest.log"
true > "$log" # Clear log file at start
...
if [[ "$ARCH" = *x86_64* ]]; then
	...
	echo -e "Architecture $ARCH might have limited support." | tee -a "$log"
	ARCH="unknown"
fi

# test if the host has IPv4/IPv6 connectivity
[[ ! -z "$LOCAL_CURL" ]] && IP_CHECK_CMD="curl -s -m 4" || IP_CHECK_CMD="wget -qO- -T 4"
IPV4_CHECK=$((ping -4 -c 1 -W 4 ipv4.google.com >"$NULL" 2>&1 && echo true) || $IP_CHECK_CMD -4 icanhazip.com 2> "$NULL")
IPV6_CHECK=$((ping -6 -c 1 -W 4 ipv6.google.com >"$NULL" 2>&1 && echo true) || $IP_CHECK_CMD -6 icanhazip.com 2> "$NULL")
if [[ -z "$IPV4_CHECK" && -z "$IPV6_CHECK" ]]; then
	echo -e
	echo -e "Warning: Both IPv4 AND IPv6 connectivity were not detected. Check for DNS issues..." | tee -a "$log"
fi
